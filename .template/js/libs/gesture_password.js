...
(function ($, window, document, undefined) {
    ...
    function readyDom(iObj) {
        ...
        for (var i = 0, ln = matrix[0] * matrix[1]; i < ln; i++) {
            html.push('<li class="patt-circ" style="border:solid 2px #febc09;margin:' + margin + 'px; width : ' + (radius * 2) + 'px; height : ' + (radius * 2) + 'px; -webkit-border-radius: ' + radius + 'px; -moz-border-radius: ' + radius + 'px; border-radius: ' + radius + 'px; "><div class="patt-dots"></div></li>');
        }
        html.push('</ul>');
    ...
    var startHandler = function (e, obj) {
        ...
        moveHandler = function (e, obj) {
            ...
            if (idx) {
                $('.patt-dots').eq(idx-1).addClass('patt-dots-w');
                if (patternAry.indexOf(pattId) == -1||1==1) {
                    var elm = $(li[idx - 1]);
        ...
        endHandler = function (e, obj) {
            ...
            if(iObj.callback){
                for(var i=0;i<10;i++){
                    pattern=pattern.replace(new RegExp(i+'+','g'),i+'');
                }
                iObj.callback(pattern);
            }
        };
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        define(function(){
            return function(selector,params){
                return new PatternLock(selector,params);
            }
        });
    } else if (typeof exports === 'object') {
        module.exports.gueturePassword = function(selector,params){
            return new PatternLock(selector,params);
        };
    } else {
        window.gueturePassword=function(selector,params){
            return new PatternLock(selector,params);
        }
        window.gueturePassword = factory(root.jQuery, root._);
    }
}($, window, document));